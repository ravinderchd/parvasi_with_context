...
import { StyleSheet, ScrollView, View, Text, Pressable, Dimensions, Image, ActivityIndicator,FlatList } from 'react-native';
...
// let encodedAuth = new Buffer("Inspector Craig Young called the incident â€œquite extraordinary.â€").toString("base64");
...
const EnglishNewsDetails = ({ route, navigation }) => {
      //console.log("Parameteres received in News Details===",route.params.newsid)
 ...
 useEffect(() => {
       let wrd = replaceSpecialCharacters('JäváSçrîptMM â€');
console.log("encoded====", wrd)
         let unmounted = false;
          let source = axios.CancelToken.source();  
        const newUrl = "http://parvasi.newunlimitedhosting.21gtech.com/services.asmx/Get_English_News_Detail";
        const adUrl =  "http://parvasi.newunlimitedhosting.21gtech.com/services.asmx/Ad_Get_Ads_By_Position";
         const requestOne = axios.post(newUrl, {News_Id: newsid, timeout: 60 , cancelToken: source.token},  
          ...
          const requestTwo = axios.post(adUrl, {Ad_Position_Id: 13}); //Dashboard Ad
        //News_Count
                  ...
                  const responseTwo = responses[1];
                    if (!unmounted) {
                         if(responseOne)
                         {
                             ...
                             console.log("RStatus===", responseOne)
                              if(status==1)
                              {
...
setBottomAd(false);
 clearTimeout(timeout);
  ...
  const cleanDesc = (txt) => {
    //const regex = /(<([^>]+)>)/gi;
     let m= txt.replace('â€œ', '“').replace("â€",'”').replace("â€™", '’').replace('â€œ','“');
     
     return m.replace('Â','').replace('â€','”').replace('Â','');
           // .replace('&nbsp;','')
           // .replace(regex,'')
  }
    ...
    <View style={{flex:1,flexDirection: 'column'}}>
      {
       isLoading ? <ActivityIndicator size="large"  style={{
        ...
        <View style={{height:height, width:width, paddingBottom:70}}>
         <WebView 
        
          source={{html: `<html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta charset="utf-8"></head><body><p>${(newsDetails[0].content)}</body></html>`  }} style={{fontSize:22}} /> 
        
         {/* <RenderHtml
      contentWidth={width}
      source={{html:newsDetails[0].content}}
    /> */}
        </View>
...
export default EnglishNewsDetails;
