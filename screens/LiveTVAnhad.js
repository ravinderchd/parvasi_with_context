...
import { View, StyleSheet, Button, Dimensions, Text, ScrollView, Image } from 'react-native';
...
const LiveTVAnhad = ({ route, navigation}) => {
...
useEffect(() => {
         ...
         let unmounted = false;
          ...
          const requestOne = axios.post(adUrl, {Ad_Position_Id: 6}); //Dashboard Ad
          axios
              .all([ requestOne])
              .then(
  ...
  return (
    ...
    <ScrollView style={{flex:1, flexDirection:'column'}}>
      ...
      injectedJavaScript={`
         ...
         `}
        source={{ html: `<iframe src="https://main.clickstreamcdn.com/parvasi-anhaad-tv/playlist.m3u8"  height=100% width=100% preload="none">`}}
      />
      {/* <WebView
        style={{ marginTop: 2,  }}
        javaScriptEnabled={true}
        domStorageEnabled={true}
        source={{ uri: `https://main.clickstreamcdn.com/parvasi-anhaad-tv/playlist.m3u8`}}
      /> */}
      </View>
      {
        adData? adData.map((v,index)=> <View key={index} style={{marginBottom:6}}><Image source={{uri:v.Ad_image_url}} style={{height:127, width:width}}></Image></View>): null
      }
          {/* <View>
